...
public class LottoService {
	 ...
	 * 4. Order객체를 생성한다.
	 * 5. Order객체에 회차번호, 주문번호, 로또번호리스트를 저장한다.
	 ...
	 * 2-1. 반복시작
	 * 3. 1 ~ 45 범위내의 숫자를 발생시킨다.
	 * 4. 이번 회차 로또 당첨번호에 3번에서 획득한 숫자가 포함되어 있는지 확인하고,
	 *    포함되어 있지 않으면 해당 숫자를 보너스번호로 반환한다. 
	 * 2-2. 반복종료
	...
	/**
	 * 로또추첨번호정보와 구매한 로또번호세트목록을 전달받아서 당첨여부를 체크하고 그 결과를 반환한다.
	 * 1. 각 로또번호세트별 당첨정보를 저장할 ArrayList객체를 생성한다.
	 * 2. 각 로또번호세트별로 당첨여부를 검사하는 반목을 시작한다.
	...
	private List<Winning> check(Lotto lotto, List<Set<Integer>> numbers) {
		...
		for (Set<Integer> set : numbers) {
			...
			winning.setWin(true);
			// check(추첨번호, 구매한로또번호세트)를 실행해서 맞은 번호를 포함하는 Set 객체를 획득하기
